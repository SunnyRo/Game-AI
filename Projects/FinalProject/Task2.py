class node(object):
    ...
    def __str__(self):
        return "level = {}, profit = {}, weight = {}, bound = {}".format(
            self.level, self.profit, self.weight, self.bound
        )


def bound(node):
    """
    @brief: calculate the bound for the node
    @param node: the node thats wanted to find the bound
...
def knapsack(n, profit, weight, capacity):
    """
    @brief: calculate the maximum profit
